...
class ls_colors(ColorScheme):
    def __init__(self):
        ...
        for key in self.ls_colors_extensions + special_keys:
        ...
        for key in [k for k in self.ls_colors_keys if '.*' not in k]:
            if key[0] == 'ex':
                ...
                self.tup_ls_colors += [('orphan', key[1])]
            elif key[0] == 'ow':
                self.tup_ls_colors += [('other_writable', key[1])]
    ...
    def new_hook_before_drawing(self, fsobject, color_list):
        if fsobject.is_directory and fsobject.exists and fsobject.get_permission_string()[8] == 'w':
            color_list.append("other_writable")

        if fsobject.is_link and not fsobject.exists:
            color_list.append("orphan")
